...
class Caixa extends AbstractController
{
    ...
    {
        /** @var CaixaService $caixaService */
        $caixaService = $this->getService(CAIXA_SERVICE);
        /** @var Session $session */
        $session = new Session();
        ...
        if (!empty($_POST)) {
            ...
            $this->result = $caixaService->PesquisaAvancada($Condicoes);
            $session->setSession(PESQUISA_AVANCADA, $Condicoes);
        } else {
            $this->result = $caixaService->PesquisaTodos($Condicoes);
        }
    }
