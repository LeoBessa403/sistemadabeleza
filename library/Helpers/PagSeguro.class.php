...
class PagSeguro
{
    private $email = "leodjx@hotmail.com";

    //URL OFICIAL
    private $url;
    private $url_redirect;
    public  $url_pagamento;
    ...
    public function __construct()
    {
        if (PROD) {
            ...
            $this->url_pagamento = "https://pagseguro.uol.com.br/checkout/v2/payment.html";
            ...
            $this->token = "538894ED307549EEA981CE4F5FC368FC";
        } else {
            //URL SANDBOX
            ...
            $this->url_pagamento = "https://sandbox.pagseguro.uol.com.br/checkout/v2/payment.html";
            ...
            $this->token = "5FDD35645CC1412296CE57A3542E48D0";
        }
        $this->email_token = "?email=" . $this->email . "&token=" . $this->token;
        $this->url .= $this->email_token;
    }

    public function solicitarPagamento($dados)
    {
        //Configurações
        ...
        //Itens
        $data['itemId1'] = $dados[CO_PLANO];
        $data['itemAmount1'] = number_format($dados[NU_VALOR], 2, ".", "");
        $data['itemQuantity1'] = '1';
        $data['itemDescription1'] = utf8_decode($dados[DS_DESCRICAO]);

        //Dados do pedido
        $data['reference'] = AssinanteService::getCoAssinanteLogado();
    ...
    public function getStatusByReference($reference)
    {
        ...
        $xml   = simplexml_load_string($transaction, 'SimpleXMLElement', LIBXML_NOCDATA);

        $array = json_decode(json_encode($xml), TRUE);

        if(!empty($array['error']) > 0) {
            //Insira seu código avisando que o sistema está com problemas
            var_dump($array);
        }
        //print_r($transaction_obj);
        if(isset($array['transactions']['transaction']['status']))
            return $array['transactions']['transaction']['status'];
        else
            return NULL;
    }
